...
### 3.6.2 신경망의 추론 처리
이 신경망은 입력층 뉴런을 784개(이미지 크기가 28x28=784), 출력층 뉴런을 10개(0~9까지 숫자 구분)로 구성한다. 그리고 은닉층은 총 두 개로, 첫 번째 은닉층에는 50개의 뉴런, 두 번째 은닉층에는 100개의 뉴런을 배치할 것이다. 여기서 50과 100은 임의로 정한 값이다.  
```python
def get_data() :
...
```  
`np.argmax() 함수로 배열에서 값이 가장 큰(확률이 가장 높은) 원소의 인덱스를 구한다. 이게 예측 결과다. 그리고 신경망이 예측한 답변과 정답 레이블을 비교하여 맞힌 숫자(accuracy_cnt)를 세고, 이를 전체 이미지 숫자로 나눠 정확도를 구한다. 코드를 실행하면 올바르게 분류한 비율이 93.52% 임을 알 수 있다.    
이 예에서 load_mnist 함수의 인수인 normalize를 True로 설저앻ㅆ다. 0~255 범위인 각 픽셀의 값을 0.0~1.0 범위로 변환한다(픽셀/255). 이처럼 데이터를 특정 범위로 변환하는 처리를 **정규화**라 하고, 신경망의 입력 데이터에 특정 변환을 가하는 것을 **전처리**라고 한다. 여기서 입력 이미지 데이터에 대한 전처리 작업으로 정규화를 수행한 셈이다.    
...
for i in range(0, len(x), batch_size) :
    ...
    accuracy_cnt += np.sum(p == t[i:i+batch_size])  # True/False로 구성된 배열에서 True 개수 측정

print("Accuracy:" + str(float(accuracy_cnt) / len(x)))
```    

`asix=1` 인수는 100x10의 배열 중 1번째 차원을 구성하는 각 원소에서 최댓값의 인덱스를 찾도록 한 것이다. asix의 예시는 다음과 같다.  
```python
